...
namespace agora.KTV
{
    public class CharacterSelect : NetworkBehaviour
    {
        ...
        public void Select()
        {
            ...
            {
                playerName = (uint) _random.Next(10000, 67889);
            }
            else
            {
                playerName = (uint) _random.Next(67891, 10000000);
            }
            CmdSelect(playerName, currentCharacterIndex);
            characterSelectDisplay.SetActive(false);
        }

        [Command(requiresAuthority = false)]
        public void CmdSelect(uint playerName, int characterIndex, NetworkConnectionToClient sender = null)
        {
            GameObject characterInstance = Instantiate(characters[characterIndex].GameplayCharacterPrefab, SpawnPoint.position, SpawnPoint.rotation);
            characterInstance.GetComponent<AgoraKTV>().playerName = playerName;
